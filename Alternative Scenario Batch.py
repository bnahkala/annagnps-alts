...
# Utilized 03.09.2020 to add scen 18-21
#
# REF 1: C:\AGNPS_Watershed_Studies\Model Development Manager.xlsx
#
# =======================================================================
#
# IMPORT =============================
...
volume_data = numpy.empty([duration, 45])
...
for x in pothole_names:
    ...
    for y in scenario_numerical:
        # SCENARIO FILE NAMES
        header = "Data Section ID, File Name"
        annID = 'AnnAGNPS ID,.\simulation\\annaid.csv'
        cell = 'Cell Data,.\watershed\AnnAGNPS_Cell_Data_Section_' + str(y) + '.csv'
        ...
        mngf = "Management Field Data,.\general\AnnAGNPS_Management_Field_Data_" + str(y) + ".csv"
        mngo = "Management Operation Data,.\general\AnnAGNPS_Management_Operation_Data_Template.csv"
        mngs = "Management Schedule Data,.\general\AnnAGNPS_Management_Schedule_Data_" + str(y) + ".csv"
        ...
        wetland = "Wetland Data,.\watershed\AnnAGNPS_Wetland_Data_Section_" + str(y) + ".csv" 
        ...
        for m in range(11, 9865):
            if (wetread[m, 0] >= start and wetread[m, 0] <= end):
                ...
                m = m + 1
                j= j + 1 
        numpy.savetxt(output_file, volume_data, delimiter=",")
    z = z + 1
